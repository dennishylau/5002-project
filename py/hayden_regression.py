...
class ConvModel(nn.Module):
    ...
    def __init__(self, linear_size=960):
        super(ConvModel, self).__init__()
        self.conv_left = nn.Conv1d(1, 8, 5)
        self.conv_right = nn.Conv1d(1, 8, 5)
...
model = ConvModel()
compute_loss = nn.L1Loss(reduction='sum')
...
for j in range(num_epoch):
    ...
    for i in range(0, len(y) - batch_size, batch_size):
        ...
        optimizer.step()
        loss_sum += float(loss)
    print(f'Epoch Loss: {loss_sum / len(y)}')
